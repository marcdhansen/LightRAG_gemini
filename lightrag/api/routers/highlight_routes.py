...
def create_highlight_routes(api_key: Optional[str] = None):
    ...
    class HighlightRequest(BaseModel):
        ...
        threshold: float = Field(0.6, description="Relevance threshold (0.0 to 1.0)", ge=0.0, le=1.0)

    class HighlightResponse(BaseModel):
        highlighted_sentences: List[str] = Field(..., description="List of sentences that match the query above the threshold")
        sentence_probabilities: List[float] = Field(..., description="Confidence scores for each highlighted sentence")

    @router.post(
        "", 
        response_model=HighlightResponse,
        dependencies=[Depends(get_combined_auth_dependency(api_key))] if api_key else []
    )
