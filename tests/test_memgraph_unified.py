...
async def test_memgraph_unified_search():
    ...
    await graph_storage.upsert_node("Project X", {"entity_id": "Project X", "entity_type": "Project", "description": "A secret project about AI."})
    await graph_storage.upsert_node("Chunk 1", {"entity_id": "Chunk 1", "entity_type": "Chunk", "content": "This chunk discusses artificial neural networks."})
    ...
    async with graph_storage._driver.session(database=graph_storage._DATABASE) as session:
        v_count = await session.run(f"MATCH (n:`{vector_storage.label}`) RETURN count(n) as c")
        ...
        rel_res = await session.run("MATCH (n:unified_ws)-[r]->(m:unified_ws) RETURN n.entity_id as src, type(r) as type, m.entity_id as tgt")
    ...
    async with graph_storage._driver.session(database=graph_storage._DATABASE) as session:
        ...
        async for r in res:
            if r['p.entity_id']:
                print(f"Found: {r['v.id']} attached to {r['p.entity_id']} with score {r['score']:.4f}")
                found = True
            else:
