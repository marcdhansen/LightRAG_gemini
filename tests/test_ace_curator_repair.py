...
async def test_ace_curator_merge_logic():
    ...
    mock_gen_response = {
        ...
        "context_data": {
            "entities": [
                {"entity_name": "AI", "entity_type": "ORGANIZATION", "description": "Short for Artificial Intelligence"},
                {"entity_name": "Artificial Intelligence", "entity_type": "TECHNOLOGY", "description": "Human-like intelligence in machines"}
            ],
            "relationships": [],
            "chunks": [{"content": "AI and Artificial Intelligence are the same thing."}]
        }
    }
    ...
    rag.ace_reflector.reflect = AsyncMock(return_value=["Insight: Deduplicate AI terms"])
...
async def test_curator_apply_repairs_execution():
    ...
    repairs = [
        {"action": "delete_relation", "source": "A", "target": "B", "reason": "Hallucination"},
        {"action": "delete_entity", "name": "C", "reason": "Not in text"},
        {"action": "merge_entities", "sources": ["D"], "target": "E", "reason": "Duplicate"}
    ]
