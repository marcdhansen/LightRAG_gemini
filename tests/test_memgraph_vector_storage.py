...
async def test_memgraph_vector_storage_basic():
    ...
    try:
        ...
        async with storage._driver.session(database=storage._DATABASE) as session:
            ...
            modules_res = await session.run("CALL mg.procedures() YIELD name, signature, returns")
        ...
        # Test upsert
        test_data = {
            "id_1": {"content": "The quick brown fox jumps over the lazy dog", "meta1": "v1"},
            "id_2": {"content": "Artificial intelligence is transforming the world", "meta1": "v2"}
        }
